 ...
 */
 ...
 function getServerTime(){
  ...
  // Set the url
  curl_setopt($ch, CURLOPT_URL,'https://api.btcturk.com/api/v2/server/time');
  // Execute
  $result=curl_exec($ch);
  // Closing
  curl_close($ch);
...
class BtcTurkPRO
{
    ...
    private function get_call($method, $params = array(), $apiKey = 0, $postMethod = 0)
    {
        ...
        $n_on = $serverTime['serverTime'];
      
           
        if ($apiKey == 1) {
            usleep(100000);
            $message = $this->apiKey.($n_on);
    ...
    {
        $query = $this->get_call('api/v2/server/time');
        return  $query->data;
