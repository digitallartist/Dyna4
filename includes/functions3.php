...
*/
    ...
    foreach( @$datas as $data){
        $wallet_params[$exchange][$data->currency]['whole_balance']=$data->whole_balance;
        $wallet_params[$exchange][$data->currency]['avaiable_balance']=$data->avaiable_balance;
        $wallet_params[$exchange][$data->currency]['reserved_balance']=$data->reserved_balance;
        $wallet_params[$exchange][$data->currency]['taker_fee']=$data->taker_fee;
        $wallet_params[$exchange][$data->currency]['maker_fee']=$data->maker_fee;
      }
      ...
      //   echo($order['id']);
             if($order['id']>0) {
               ...
               if($open_orders[$exchange][$i]['type']=="sell"){
                  $wallet_params[$exchange][$pre_coin]['avaible_balance']-=$order['amount'];
                  $wallet_params[$exchange][$pre_coin]['reserved_balance']+=$order['amount'];
               } else {
                   $wallet_params[$exchange][$last_coin]['avaible_balance']-=$order['amount'];
          ...
          foreach (@$datas->getOrders() as $order) {
            ...
            //    $open_orders[$exchange][$order->getTxId()]['market']=$exchange;
             ...
             if($open_orders[$exchange][$i]['type']=="sell"){
                $wallet_params[$exchange][$pre_coin]['avaible_balance']-=$order->getVol()-$order->getVolExec();
                $wallet_params[$exchange][$pre_coin]['reserved_balance']+=$order->getVol()-$order->getVolExec();
             } else {
                 $wallet_params[$exchange][$last_coin]['avaible_balance']-=$order->getVol()-$order->getVolExec();
...
function get_initial_wallets($date='', $term=2, $direction=-1){
	...
	global $bas_usd_eq;
			...
			$dbhost = 'localhost';
           $dbuser = 'boto_erp';
           $dbpass = 'rXC8qlB6xHG7EO2v';
           $dbname = 'musterdi_koin';
           $conn = mysqli_connect($dbhost, $dbuser, $dbpass,$dbname);
		   
	
	//$q_ek=" WHERE fld_active='" . $term . "' ";
	//if($date!='')
		$q_ek=" WHERE record_time>='2019-06-17 14:20:20'";
	$q="SELECT * FROM gain_control " . $q_ek .  "   ORDER BY id ASC LIMIT 1";
	//echo $q;
	$r = mysqli_query ($conn, $q); // Run the query.
  	...
  	if($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)){
		...
		$bas_try_eq=$row['try_cash_eq'];
		$bas_usd_eq=$row['usd_cash_eq'];
		
		$simulation_start_date=$row['record_time'];
		//if($direction=-1) {
		
				make_sample_wallet('BtcTurk','TRY',$row['try_cash']);
				make_sample_wallet('BtcTurk','USD',0);
				make_sample_wallet('BtcTurk','EUR',0);
				make_sample_wallet('BtcTurk','BTC',$row['m2_btc']);
				make_sample_wallet('BtcTurk','ETH',$row['m2_eth']);
				make_sample_wallet('BtcTurk','LTC',$row['m2_ltc']);
				make_sample_wallet('BtcTurk','XRP',$row['m2_xrp']);
				make_sample_wallet('BtcTurk','XLM',$row['m2_xlm']);
				make_sample_wallet('BtcTurk','USDT',$row['m2_usdt']);
				
				make_sample_wallet('Kraken','TRY',0);
				make_sample_wallet('Kraken','USD',$row['usd_cash']);
				make_sample_wallet('Kraken','EUR',$row['eur_cash']);
				make_sample_wallet('Kraken','BTC',$row['m1_btc']);
				make_sample_wallet('Kraken','ETH',$row['m1_eth']);
				make_sample_wallet('Kraken','LTC',$row['m1_ltc']);
				make_sample_wallet('Kraken','XRP',$row['m1_xrp']);
				make_sample_wallet('Kraken','XLM',$row['m1_xlm']);
				make_sample_wallet('Kraken','USDT',$row['m1_usdt']);
		/*	} else {
	
				make_sample_wallet('BtcTurk','TRY',$row['try_cash']);
				make_sample_wallet('BtcTurk','USD',0);
				make_sample_wallet('BtcTurk','EUR',0);
				make_sample_wallet('BtcTurk','BTC',$row['btc_balance']);
				make_sample_wallet('BtcTurk','ETH',$row['eth_balance']);
				make_sample_wallet('BtcTurk','LTC',$row['ltc_balance']);
				make_sample_wallet('BtcTurk','XRP',$row['xrp_balance']);
				make_sample_wallet('BtcTurk','XLM',$row['xlm_balance']);
				make_sample_wallet('BtcTurk','USDT',$row['usdt_balance']);
				
				make_sample_wallet('Kraken','TRY',0);
				make_sample_wallet('Kraken','USD',$row['usd_cash']);
				make_sample_wallet('Kraken','EUR',0);
				make_sample_wallet('Kraken','BTC',0);
				make_sample_wallet('Kraken','ETH',0);
				make_sample_wallet('Kraken','LTC',0);
				make_sample_wallet('Kraken','XRP',0);
				make_sample_wallet('Kraken','XLM',0);
				make_sample_wallet('Kraken','USDT',0);
		}*/
	//}
...
function make_sample_wallet($exchange,$currency,$amount){

	global $wallet_params;
	
	    $wallet_params[$exchange][$currency]['whole_balance']=$amount;
        $wallet_params[$exchange][$currency]['avaiable_balance']=$amount;
        $wallet_params[$exchange][$currency]['reserved_balance']=0;
	
	 if($exchange=='BtcTurk') {
        $wallet_params[$exchange][$currency]['taker_fee']=0.002;
        $wallet_params[$exchange][$currency]['maker_fee']=0.002;
 
    } else if($exchange=='Kraken') {
        $wallet_params[$exchange][$currency]['taker_fee']=0.0016;
        $wallet_params[$exchange][$currency]['maker_fee']=0.0016;
    }
	$wallet_params[$exchange][$currency]['minumum_order']=0;
	
	if($currency=='BTC')
		$wallet_params[$exchange][$currency]['minumum_order']=0.002;
	if($currency=='ETH')
		$wallet_params[$exchange][$currency]['minumum_order']=0.02;
	if($currency=='LTC')
		$wallet_params[$exchange][$currency]['minumum_order']=0.1;
	if($currency=='XRP')
		$wallet_params[$exchange][$currency]['minumum_order']=40;
	if($currency=='XLM')
		$wallet_params[$exchange][$currency]['minumum_order']=40;
...
function start_simulator($normal_min_rate, $reverse_min_rate, $start_date, $end_date=''){
	echo "simulasyon başladı. <br/>";
	global $wallet_params;
	global $usd_try;
	global $eur_try;
	...
	$q_ek="";
	if($end_date!="")
		$q_ek .=" AND end_date<='" . $end_date . "' ";
	
	$q="SELECT * FROM cycle_results WHERE (rate>=" . $normal_min_rate . " OR rev_rate>= " . $reverse_min_rate . " )  AND rate!='' AND rev_rate!=''  AND entry>='" . $start_date . "'" .  $q_ek . " LIMIT 30000";
//echo $q;
	$r = mysqli_query ($conn, $q); // Run the query.
	...
	$j=0;	
  	while($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)){
		...
		$i++;
		$action_completed=0;
		$pair2_1=$result = substr($row['pairs2'], 0, 3);
		$pair2_2=$result = substr($row['pairs2'], -3);
		$pair1_1=$result = substr($row['pairs1'], 0, 4);
		$pair1_2=$result = substr($row['pairs1'], -4);
		$pair1_2_a=$result = substr($row['pairs1'], -3);
		if($row['rate']>=$normal_min_rate) {
				//BTCTURK'de sat
			//KRAKENde al
			$usd_try=$row['usd_tl'];
			$eur_try=$row['eur_tl'];
			
			$market2_act_volume=($wallet_params['BtcTurk'][$pair2_2]['avaiable_balance']/$row['market2_ask_price'])*0.95;
			$market1_act_volume=($wallet_params['Kraken'][$pair2_1]['avaiable_balance']/$row['market1_bid_price'])*0.95;
			
			$volume=min($row['market2_ask_volume']*0.5,$row['market1_bid_volume']*0.85,$market2_act_volume,$market1_act_volume);
			$market2_cost=$volume*$row['market2_ask_price']*(1-$wallet_params['BtcTurk'][$pair2_1]['maker_fee']);
			$market1_cost=$volume*$row['market1_bid_price']*(1+$wallet_params['Kraken'][$pair2_1]['maker_fee']);
			
			
			
			
			$cycle_do_it=true;
		//	echo $wallet_params['BtcTurk'][$pair2_2]['whole_balance'] . " : ".  $wallet_params['Kraken'][$pair2_1]['whole_balance'] . " : " .$volume. "<br/>";
			
			if($volume<$wallet_params['BtcTurk'][$pair2_1]['minumum_order']){
				$cycle_do_it=false;
				//echo " false 1 : " .  $pair2_1 . $pair2_2 . " ::: " . $volume . " < " .$wallet_params['BtcTurk'][$pair2_1]['minumum_order'] ."<br/>";
			}
			if($market1_cost>$wallet_params['Kraken'][$pair1_2_a]['avaiable_balance']) {
				$cycle_do_it=false;
					//echo " false 2 : " . $market2_cost . " < ". $wallet_params['BtcTurk'][$pair2_2]['avaiable_balance'] ."<br/>";
			}
			if($volume>$wallet_params['BtcTurk'][$pair2_1]['avaiable_balance']) {
				$cycle_do_it=false;
			
			//	echo " false 3 : " . $wallet_params['Kraken'][$pair2_1]['avaiable_balance'] ."<br/>";
			}
			
			if($cycle_do_it==true) {
				$j++;
				$wallet_params['BtcTurk'][$pair2_2]['avaiable_balance']=$wallet_params['BtcTurk'][$pair2_2]['avaiable_balance']+$market2_cost;
				$wallet_params['Kraken'][$pair1_2_a]['avaiable_balance']=$wallet_params['Kraken'][$pair1_2_a]['avaiable_balance']-$market1_cost;
				$wallet_params['BtcTurk'][$pair2_1]['avaiable_balance']=$wallet_params['BtcTurk'][$pair2_1]['avaiable_balance']-$volume;
				$wallet_params['Kraken'][$pair2_1]['avaiable_balance']=$wallet_params['Kraken'][$pair2_1]['avaiable_balance']+$volume;
				//echo $j. " -> $i. düz işlem. Oran : " . $row['rate'] . ", Hacim : " . $volume . " : " .   $pair2_1.$pair2_2 . " ". $row['entry'].  "<br/>";
			}	
			
		} 
		
		if($row['rev_rate']>=$reverse_min_rate) {
				//BTCTURK'de al
			//KRAKENde sat
			
			//echo $i . " ters burda " . $row['rev_rate'] . " : " ; 
			//echo $i . " ";
			$usd_try=$row['usd_tl'];
			$eur_try=$row['eur_tl'];
			
			$market2_act_volume=($wallet_params['BtcTurk'][$pair2_2]['avaiable_balance']/$row['market2_bid_price'])*0.95;
			$market1_act_volume=($wallet_params['Kraken'][$pair2_1]['avaiable_balance']/$row['market1_ask_price'])*0.95;
			
			$volume=min($row['market2_bid_volume']*0.5,$row['market1_ask_volume']*0.85,$market2_act_volume,$market1_act_volume);
			$market2_cost=$volume*$row['market2_bid_price']*(1+$wallet_params['BtcTurk'][$pair2_1]['maker_fee']);
			$market1_cost=$volume*$row['market1_ask_price']*(1-$wallet_params['Kraken'][$pair2_1]['maker_fee']);
			...
			if($market2_cost>$wallet_params['BtcTurk'][$pair2_2]['avaiable_balance']) {
				$cycle_do_it=false;
				//	echo " false 2 : " . $market2_cost . " < ". $wallet_params['BtcTurk'][$pair2_2]['avaiable_balance'] ."<br/>";
			}
			if($volume>$wallet_params['Kraken'][$pair2_1]['avaiable_balance']) {
				$cycle_do_it=false;
			...
			if($cycle_do_it==true) {
				...
				$wallet_params['BtcTurk'][$pair2_1]['avaiable_balance']=$wallet_params['BtcTurk'][$pair2_1]['avaiable_balance']+$volume;
				$wallet_params['Kraken'][$pair2_1]['avaiable_balance']=$wallet_params['Kraken'][$pair2_1]['avaiable_balance']-$volume;
				echo $j. " -> $i. ters işlem. Oran : " . $row['rev_rate'] . ", Hacim : " . $volume . " : " .   $pair2_1.$pair2_2 . " ". $row['entry'].  "<br/>";
			}
			
			/*
			$volume_try2=$row['market2_bid_price']*$row['market2_bid_volume']*0.5;
			$volume_try1=$row['market1_ask_price']*$row['market2_ask_volume']*0.85;
			*/
		}
