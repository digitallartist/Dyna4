<?php
require_once("send_slack_message_v2.php");
  ...
  //Dizilere atılmış güncel cüzdan bilgilerini veritabanında günceller
    ...
    //Dizilere atılmış güncel cüzdan bilgilerini veritabanında günceller
      function calculate_wallet_rates_and_make_warnings($m1_cypto_moneys,$m2_cypto_moneys,$m1_fiat_money='ZUSD',$m2_fiat_money='TRY',$m1='Kraken',$m2='BtcTurk'){
        global $wallet_params;
        global $cur_rates;
        global $starting_rates;
        global $starting_values;
        global $fiat_money_status;
        global $cypto_money_status;
        global $alarm_rates;
        global $dbc;
...
$m1_cur_rate=1;
if($m1_fiat_money=='ZEUR' && $m2_fiat_money=='TRY')
  $m1_cur_rate=$cur_rates['EURTRY'];
else if($m1_fiat_money=='ZUSD' && $m2_fiat_money=='TRY')
  $m1_cur_rate=$cur_rates['USDTRY'];

  $m2_cur_rate=1;



$wallet_params['All'][$m2_fiat_money]['whole_balance']=$wallet_params[$m1][$m1_fiat_money]['whole_balance']*$m1_cur_rate+$wallet_params[$m2][$m2_fiat_money]['whole_balance']*$m2_cur_rate;
$try_status=0;
$main_fiat_money_status=0;
$fiat_money_status[$m2]['percent']=0;
$fiat_money_status[$m1]['percent']=0;
  echo "<br/>";  echo "<br/>";
  
  	   //#36a64f
  $attachments[0]["ts"]=$timestamp;
  $attachments[0]["color"]="#009900";
  $attachments[0]["text"]="FIAT WORKING";
  
  $attachments[1]["ts"]=$timestamp;
  $attachments[1]["color"]="#009900";
  $attachments[1]["text"]="M2 CRYPTO WORKING";
  
  $attachments[2]["ts"]=$timestamp;
  $attachments[2]["color"]="#009900";
  $attachments[2]["text"]="M1 CRYPTO WORKING";
  
  
if($wallet_params['All'][$m2_fiat_money]['whole_balance']>0) {
  $fiat_money_status[$m2]['percent']=($wallet_params[$m2][$m2_fiat_money]['whole_balance']/$wallet_params['All'][$m2_fiat_money]['whole_balance'])*100;
  $fiat_money_status[$m1]['percent']=100-$fiat_money_status[$m2]['percent'];
}
//  $starting_values['All'][$m2_fiat_money]
$fiat_must_be=$starting_values['All']['TRY']*0.95;
  $m2_fiat_must_be=$starting_values['All'][$m2_fiat_money]*$fiat_money_status[$m1]['percent']*0.95;
  $m1_fiat_must_be=$starting_values['All'][$m2_fiat_money]*$fiat_money_status[$m1]['percent']*0.95;
  
  $fiat_i=0;
  if($starting_values['All']['TRY']>0 )
{
  if($wallet_params['All'][$m2_fiat_money]['whole_balance']>$fiat_must_be)
  {
  	 if($alarm_rates[$m2]['FIAT']['alarm']>$fiat_money_status[$m2]['percent']) {
       echo $m2 . " -> " . $m2_fiat_money . " -> " . " ALARM - MANUAL DEPOSIT /  BALANCE " . " = >>" . print_ytl($alarm_rates[$m2]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m2]['percent'],2,'%');
     
        if($attachments[0]["text"]=="FIAT WORKING"){
          $attachments[0]["color"]="#990000";
          $attachments[0]["text"]="FIAT PROBLEMS"; 
        }

        $attachments[0]["fields"][$fiat_i]["title"]="ALARM - MANUAL DEPOSIT REQUIRED";
        $attachments[0]["fields"][$fiat_i]["value"]=$m2 ." -> ". $m2_fiat_money ." -> " . print_ytl($fiat_money_status[$m2]['percent'],2,'%');
        $attachments[0]["fields"][$fiat_i]["short"]=true;
        $fiat_i++;

	 } else if($alarm_rates[$m2]['FIAT']['warning']>$fiat_money_status[$m2]['percent']) {
       echo $m2 . " -> " . $m2_fiat_money . " -> " . " WARNING - MANUAL DEPOSIT /  BALANCE " . " = >>" . print_ytl($alarm_rates[$m2]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m2]['percent'],2,'%');
     
       if($attachments[0]["text"]=="FIAT WORKING"){
         $attachments[0]["color"]="990099";
         $attachments[0]["text"]="FIAT PROBLEMS"; 
       }

       $attachments[0]["fields"][$fiat_i]["title"]="WARNINNG - MANUAL DEPOSIT REQUIRED";
       $attachments[0]["fields"][$fiat_i]["value"]=$m2 ." -> ". $m2_fiat_money ." -> " . print_ytl($fiat_money_status[$m2]['percent'],2,'%');
       $attachments[0]["fields"][$fiat_i]["short"]=true;
       $fiat_i++;
     
     } else  {
       echo $m2 . " -> " . $m2_fiat_money  . " -> " . " NORMAL  " . " = >>" . print_ytl($fiat_money_status[$m2]['percent'],2,'%') . " | " . print_ytl($alarm_rates[$m2]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m2]['percent'],2,'%');
     
     
    }
     echo "<br/>";

     	 if($alarm_rates[$m1]['FIAT']['alarm']>$fiat_money_status[$m1]['percent']) {
         
    
      
          echo $m1 . " -> " . $m1_fiat_money . " -> " . " ALARM - MANUAL DEPOSIT /  BALANCE " . " = >>" . print_ytl($alarm_rates[$m1]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m1]['percent'],2,'%');
          ...
          $attachments[0]["fields"][$fiat_i]["title"]="ALARM - MANUAL DEPOSIT REQUIRED";
          $attachments[0]["fields"][$fiat_i]["value"]=$m1 ." -> ". $m1_fiat_money ." -> " . print_ytl($fiat_money_status[$m1]['percent'],2,'%');
          $attachments[0]["fields"][$fiat_i]["short"]=true;
          $fiat_i++;
        
        
        } else if($alarm_rates[$m2]['FIAT']['warning']>$fiat_money_status[$m1]['percent']) {
          echo $m1 . " -> " . $m1_fiat_money . " -> " . " WARNING - MANUAL DEPOSIT /  BALANCE " . " = >>" . print_ytl($alarm_rates[$m1]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m1]['percent'],2,'%');
          ...
          $attachments[0]["fields"][$fiat_i]["title"]="WARNING - MANUAL DEPOSIT REQUIRED";
          $attachments[0]["fields"][$fiat_i]["value"]=$m1 ." -> ". $m1_fiat_money ." -> " . print_ytl($fiat_money_status[$m1]['percent'],2,'%');
          $attachments[0]["fields"][$fiat_i]["short"]=true;
          $fiat_i++;
      
        } else  {
          echo $m1 . " -> " . $m1_fiat_money  . " -> " . " NORMAL  " . " = >>" . print_ytl($fiat_money_status[$m1]['percent'],2,'%') . " | " . print_ytl($alarm_rates[$m1]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m1]['percent'],2,'%');
        }

  } else {
    echo " TOTAL -> " . " >->->->PROBLEM - MISSING FIAT <-<-<-< " . " = >>" . print_ytl($fiat_money_status[$m2]['percent'],2,'%') . " | " . print_ytl($alarm_rates[$m2]['FIAT']['alarm'],2,'%') . " > " .  print_ytl($fiat_money_status[$m2]['percent'],2,'%');
    ...
    $attachments[0]["fields"][$fiat_i]["title"]="MISSING FIAT - CHECK THE SYSTEM";
    $attachments[0]["fields"][$fiat_i]["value"]=$m1 ." -> ". $m1_fiat_money ." -> " . print_ytl($fiat_money_status[$m1]['percent'],2,'%');
    $attachments[0]["fields"][$fiat_i]["short"]=true;
    $fiat_i++;
...
for ($i = 0; $i < count($m1_cypto_moneys); $i++) {
  echo "<br/>". $i. "<br/>";
  $cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']=0;
  $cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']=0;

  if($wallet_params[$m2][$m2_cypto_moneys[$i]]['whole_balance']>0 || $wallet_params[$m1][$m1_cypto_moneys[$i]]['whole_balance']>0) {
    $cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']=($wallet_params[$m2][$m2_cypto_moneys[$i]]['whole_balance']/($wallet_params[$m2][$m2_cypto_moneys[$i]]['whole_balance']+$wallet_params[$m1][$m1_cypto_moneys[$i]]['whole_balance']))*100;
    $cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']=100-$cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'];
  }

  $crypto_i=0;

  $m2_must_be=$starting_values[$m2][$m2_cypto_moneys[$i]]*($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']/100)*0.9;
if($starting_values['All'][$m2_cypto_moneys[$i]]>0 )
{
  if($wallet_params[$m2][$m2_cypto_moneys[$i]]['whole_balance']>$m2_must_be)
  {
    //echo $wallet_params[$m2][$m2_cypto_moneys[$i]]['whole_balance'].">" .$m2_must_be . "-----------" . $starting_values[$m2][$m2_cypto_moneys[$i]] . " * " . $cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'] . " ***"  ;
    if($alarm_rates[$m2]['CYPTO']['alarm']>$cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']) {
      echo $m2 . " -> " . $m2_cypto_moneys[$i] . " -> " . " ALARM - WITHDRAW WAS STARTED AUTOMATICLY " . " = >>" . print_ytl($alarm_rates[$m2]['CYPTO']['warning'],2,'%') . " > " .  print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
    
      if($attachments[1]["text"]=="M2 CRYPTO WORKING"){
        $attachments[1]["color"]="#990000";
        $attachments[1]["text"]=$m2 . " CRYPTO PROBLEMS"; 
      }

      $attachments[1]["fields"][$crypto_i]["title"]="ALARM - WITHDRAW WAS STARTED AUTOMATICLY";
      $attachments[1]["fields"][$crypto_i]["value"]=$m2 ." -> ". $m2_cypto_moneys[$i] ." -> " . print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
      $attachments[1]["fields"][$crypto_i]["short"]=false;
      $crypto_i++;
    
    } else if($alarm_rates[$m2]['CYPTO']['warning']>$cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']) {
      echo $m2 . " -> " . $m2_cypto_moneys[$i] . " -> " . " WARNING - AUTO WITHDRAW CAN FIX THE PROBLEM" . " = >>" . print_ytl($alarm_rates[$m2]['CYPTO']['alarm'],2,'%') . " > " .  print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
      ...
      $attachments[1]["fields"][$crypto_i]["title"]="WARNING - AUTO WITHDRAW CAN FIX THE PROBLEM";
      $attachments[1]["fields"][$crypto_i]["value"]=$m2 ." -> ". $m2_cypto_moneys[$i] ." -> " . print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
      $attachments[1]["fields"][$crypto_i]["short"]=false;
      $crypto_i++;
      ...
      echo $m2 . " -> " . $m2_cypto_moneys[$i] . " -> " . " NORMAL  " . " = >>" . print_ytl($alarm_rates[$m2]['CYPTO']['warning'],2,'%') . " | " . print_ytl($alarm_rates[$m2]['CYPTO']['alarm'],2,'%') . " | " .  print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
    }
  } else {
    echo $m2 . " -> " . $m2_cypto_moneys[$i] . " -> " . " >->->->PROBLEM - MISSING COINS <-<-<-< " . " = >>" . print_ytl($alarm_rates[$m2]['CYPTO']['warning'],2,'%') . " | " . print_ytl($alarm_rates[$m2]['CYPTO']['alarm'],2,'%') . " | " .  print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
    ...
    $attachments[1]["fields"][$crypto_i]["title"]="PROBLEM - MISSING COINS";
    $attachments[1]["fields"][$crypto_i]["value"]=$m2 ." -> ". $m2_cypto_moneys[$i] ." -> " . print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
    $attachments[1]["fields"][$crypto_i]["short"]=false;
    $crypto_i++;
  ...
  echo "<br/>";
  $crypto_i2=0;
  $m1_must_be=$starting_values[$m1][$m1_cypto_moneys[$i]]*($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']/100)*0.9;
  if($starting_values['All'][$m1_cypto_moneys[$i]]>0 )
{
    if($wallet_params[$m1][$m1_cypto_moneys[$i]]['whole_balance']>$m1_must_be)
    {
      if($alarm_rates[$m1]['CYPTO']['alarm']>$cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']) {
        echo $m1 . " -> " . $m1_cypto_moneys[$i] . " -> " . " ALARM -  MANUAL WITHDRAW REQUIRED" . " = >>" . print_ytl($alarm_rates[$m1]['CYPTO']['warning'],2,'%') . " > " .  print_ytl($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent'],2,'%');
       if($attachments[2]["text"]=="M1 CRYPTO WORKING"){
          $attachments[2]["color"]="#990000";
          $attachments[2]["text"]=$m1 . " CRYPTO PROBLEMS"; 
        }
    
        $attachments[2]["fields"][$crypto_i2]["title"]="ALARM - MANUAL WITHDRAW REQUIRED";
        $attachments[2]["fields"][$crypto_i2]["value"]=$m1 ." -> ". $m1_cypto_moneys[$i] ." -> " . print_ytl($cypto_money_status[$m1][$m21cypto_moneys[$i]]['percent'],2,'%');
        $attachments[2]["fields"][$crypto_i2]["short"]=false;
        $crypto_i2++;
    
      } else if($alarm_rates[$m1]['CYPTO']['warning']>$cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']) {
        echo $m1 . " -> " . $m1_cypto_moneys[$i] . " -> " . " WARNING - MANUAL WITHDRAW CAN BE MADE " . " = >>" . print_ytl($alarm_rates[$m1]['CYPTO']['alarm'],2,'%') . " > " .  print_ytl($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent'],2,'%');
        if($attachments[2]["text"]=="M1 CRYPTO WORKING"){
          $attachments[2]["color"]="#990000";
          $attachments[2]["text"]=$m1 . " CRYPTO PROBLEMS"; 
        }
    
        $attachments[2]["fields"][$crypto_i2]["title"]="WARNING - MANUAL WITHDRAW CAN BE MADE";
        $attachments[2]["fields"][$crypto_i2]["value"]=$m1 ." -> ". $m1_cypto_moneys[$i] ." -> " . print_ytl($cypto_money_status[$m1][$m21cypto_moneys[$i]]['percent'],2,'%');
        $attachments[2]["fields"][$crypto_i2]["short"]=false;
        $crypto_i2++;
      
      } else  {
        echo $m1 . " -> " . $m1_cypto_moneys[$i] . " -> " . " NORMAL  " . " = >>" . print_ytl($alarm_rates[$m1]['CYPTO']['warning'],2,'%') . " | " . print_ytl($alarm_rates[$m1]['CYPTO']['alarm'],2,'%') . " | " .  print_ytl($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent'],2,'%');
        
  
      
      }
     } else {
      echo $m1 . " -> " . $m1_cypto_moneys[$i] . " -> " . "  >->->->PROBLEM! - MISSING COINS<-<-<-<  " . " = >>" . print_ytl($alarm_rates[$m1]['CYPTO']['warning'],2,'%') . " | " . print_ytl($alarm_rates[$m1]['CYPTO']['alarm'],2,'%') . " | " .  print_ytl($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent'],2,'%');
      ...
      $attachments[2]["fields"][$crypto_i2]["title"]="PROBLEM - MISSING COINS";
      $attachments[2]["fields"][$crypto_i2]["value"]=$m1 ." -> ". $m1_cypto_moneys[$i] ." -> " . print_ytl($cypto_money_status[$m1][$m21cypto_moneys[$i]]['percent'],2,'%');
      $attachments[2]["fields"][$crypto_i2]["short"]=false;
      $crypto_i2++;
...
//  $attachments[0]["footer"]="Dyna Cycle API";
  //$attachments[0]["footer_icon"]="https://platform.slack-edge.com/img/default_application_icon.png";
   ...
   $attachments[0]["footer"]="Dyna Cycle API FIAT";
     $attachments[1]["footer"]="Dyna Cycle API M2";
       $attachments[2]["footer"]="Dyna Cycle API M1";
...
/*
echo "<br/>Fiat Money Status<br/>";
print_r($fiat_money_status);
echo "<br/>
</br/>Cypto Money Status<br/>
</br/>";
print_r($cypto_money_status);
*/
}
...
function get_balances_from_db($exchange='', $currency=''){
  ...
  $r = @mysqli_query ($dbc, $q); // Run the query.
      ...
      $wallet_params['All'][$row['currency']]['whole_balance']+=$row['whole_balance'];
    }
}
