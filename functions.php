...
*/
  ...
  //Dizilere atılmış güncel cüzdan bilgilerini veritabanında günceller
    ...
    //Dizilere atılmış güncel cüzdan bilgilerini veritabanında günceller
      function calculate_wallet_rates_and_make_warnings($m1_cypto_moneys,$m2_cypto_moneys,$m1_fiat_money='ZEUR',$m2_fiat_money='TRY',$m1='Kraken',$m2='BtcTurk'){
        global $wallet_params;
        global $cur_rates;
...
if($starting_values['All'][$m2_cypto_moneys[$i]]>0 )
{
  if($wallet_params[$m2][$m2_cypto_moneys[$i]]['whole_balance']>$m2_must_be)
  {
    ...
    if($alarm_rates[$m2]['CYPTO']['alarm']>$cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']) {
      echo $m2 . " -> " . $m2_cypto_moneys[$i] . " -> " . " ALARM-MAKE WITHDRAW " . " = >>" . print_ytl($alarm_rates[$m2]['CYPTO']['warning'],2,'%') . " > " .  print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
    } else if($alarm_rates[$m2]['CYPTO']['warning']>$cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent']) {
      echo $m2 . " -> " . $m2_cypto_moneys[$i] . " -> " . " WARNING " . " = >>" . print_ytl($alarm_rates[$m2]['CYPTO']['alarm'],2,'%') . " > " .  print_ytl($cypto_money_status[$m2][$m2_cypto_moneys[$i]]['percent'],2,'%');
    } else  {
  ...
  if($starting_values['All'][$m1_cypto_moneys[$i]]>0 )
{
    if($wallet_params[$m1][$m1_cypto_moneys[$i]]['whole_balance']>$m1_must_be)
    {
      if($alarm_rates[$m1]['CYPTO']['alarm']>$cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']) {
        echo $m1 . " -> " . $m1_cypto_moneys[$i] . " -> " . " ALARM-AUTO WITHDRAW " . " = >>" . print_ytl($alarm_rates[$m1]['CYPTO']['warning'],2,'%') . " > " .  print_ytl($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent'],2,'%');
      } else if($alarm_rates[$m1]['CYPTO']['warning']>$cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent']) {
        echo $m1 . " -> " . $m1_cypto_moneys[$i] . " -> " . " WARNING " . " = >>" . print_ytl($alarm_rates[$m1]['CYPTO']['alarm'],2,'%') . " > " .  print_ytl($cypto_money_status[$m1][$m1_cypto_moneys[$i]]['percent'],2,'%');
      } else  {
...
function get_start_withdraw_value_from_db($exchange='', $currency=''){
  global $wallet_params;
  global $dbc;


 $wallet_params[$exchange][$currency]['withdraw_start_value']=0;
$wallet_params[$exchange][$currency]['withdraw_wallet_name']='';
$wallet_params[$exchange][$currency]['entry']='';
  $q_ek=" WHERE 1=1";
  if($exchange!="")
    $q_ek .=" AND market='" . $exchange . "'";
  if($currency!="")
  ...
  $r = @mysqli_query ($dbc, $q); // Run the query.
  	while($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)){
      $wallet_params[$row['market']][$row['currency']]['withdraw_start_value']=$row['withdraw_start_value'];
	   $wallet_params[$row['market']][$row['currency']]['withdraw_wallet_name']=$row['withdraw_wallet_name'];
	   	   $wallet_params[$row['market']][$row['currency']]['entry']=$row['entry'];
...
function get_api_quotas($publisher){
  global $dbc;
  $q="SELECT fera.account_key, fep.check_quota_url FROM fiat_exchange_rate_apis fera
  LEFT JOIN fiat_exchange_publisher fep ON fep.publisher LIKE fera.publisher
   WHERE fera.publisher='" . $publisher . "'";
  $r = @mysqli_query ($dbc, $q); // Run the query.
  while($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)) {
    update_api_quotas($publisher, $row['account_key'], $row['check_quota_url']);
  }
}

function update_api_quotas($publisher, $account_key, $check_quota_url){
  global $dbc;
      $url=$check_quota_url  . $account_key;
      $inputJSON = file_get_contents($url);
      $inputs= json_decode( $inputJSON, TRUE ); //convert JSON into array


      if($publisher=='1Forge') {
        $quota_used=$inputs['quota_used'];
        $quota_limit=$inputs['quota_limit'];
        $quota_remaining=$inputs['quota_remaining'];
        $hours_until_reset=$inputs['hours_until_reset'];

        $qi="UPDATE fiat_exchange_rate_apis SET quota_used = $quota_used ,  quota_limit=$quota_limit, quota_remaining= $quota_remaining,  hours_until_reset=$hours_until_reset
...
function check_status_controls($exchange='Kraken', $currency='DASH',$pair='DASHUSD',$control_time='' ){
  global $wallet_params;
  global $dbc;
  ...
  global $btcturk_connect;
	...
	$ab=$wallet_params[$exchange][$currency]['avaiable_balance'];
	$ab_minus=$ab*0.999;
	$wsv=$wallet_params[$exchange][$currency]['withdraw_start_value'];
	$wn=$wallet_params[$exchange][$currency]['withdraw_wallet_name'];

	$last_act='';
	$q="SELECT rate, entry FROM cycle_results WHERE pairs1='".$pair."' ORDER BY id DESC LIMIT 1";
	$r = @mysqli_query ($dbc, $q); // Run the query.
	if($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)){
		$oran=$row['rate'];
		$last_act=$row['entry'];
	}
	//echo "<br/><br/>".$exchange . "->" . $currency . " İŞLEM BAŞLADI". "<br/>";
	if($control_time<$last_act){
			//echo "<br/>".$exchange . "->" . $currency . " süper çalışıyor";
	} else {
		//echo "<br/>".$exchange . "->" . $currency . " durmuş, çalışmalı!!!!!!";

	}
		if($ab>=$wsv && $wsv>0 && $wn!='') {
			$make_withdraw=1;
			//echo "<br/>". $exchange . "->" . $currency . " Acil Withdraw yapılıyor... " ;
		} else if($ab<$wsv && $wsv>0 && $wn!='') {

			//echo "<br/>". $exchange . "->" . $currency ." Bakiye Düz İçin uygun.";
		} else {
			//echo "<br/>". $exchange . "->" . $currency ." İŞLEM İÇİN UYGUN DEĞİL";
		...
		if($make_withdraw==1) {
			...
			if($exchange=='Kraken') {
				...
				try {
				$kraken_withdraw = $kraken_connect->makeWithdraw('currency',$currency,$wn,$ab_minus);
				//$kraken_withdraw = $kraken_connect->makeWithdraw('currency','DASH','Btcturk_Dash_Mustafa','1');
				$refid=$kraken_withdraw->getRefid();
				//echo "<br/>".  $exchange . "->" . $currency . " Withdraw gerçekleşti :" . $refid . "<br/><br/>";
				} catch (Exception $e) {
...
/*
	function make_auto_withdraw($exchange='', $currency=''){
		 global $wallet_params;
		 global $dbc;

		// for ($i = 0; $i < count($m1_cypto_moneys); $i++) {
			echo $wallet_params['Kraken']['XXBT']['avaiable_balance'] . " ::: ";
		echo $wallet_params['Kraken']['XXBT']['withdraw_start_value']. " ::: ";
		 echo $wallet_params['Kraken']['XXBT']['withdraw_wallet_name']. " ::: ";
        ...
        */
          ...
          */
            ...
            /*  echo "başla <br/>
              ...
              if($gain_rates['straight']['normal']>=$min_start_rates['straight']['normal'] && $gain_rates['straight']['normal']!=0){
			   ...
			   */
			   if($market_data[1]['pairs']=='XTZTRY'){
					@$target_volume=sprintf("%1.2f",min($market1_max_act, $market1_volume, $market2_max_act, $market2_volume));
					   if($target_volume>0)
                  $target_volume-=0.01;

			 // echo `XTZ` . $target_volume;
				} else {
				  @$target_volume=sprintf("%1.4f",min($market1_max_act, $market1_volume, $market2_max_act, $market2_volume));
				     if($target_volume>0)
						$target_volume-=0.0001;
                ...
                //  echo "İşlem Hacmi : " . $target_volume;
                            ...
                            //      </br/>sonnn1 : " . $wallet_params[$exchange2][$pair2A]['avaiable_balance'] . " / ". $market2_price;
                                 ///////////////////
              } else if($gain_rates['reverse']['normal']>=$min_start_rates['reverse']['normal'] && $gain_rates['reverse']['normal']!=0){
            /*    echo "2<br/>
              ...
              // echo $market_data[1]['pairs'] . " : " .$target_volume;
            ...
            //   echo "1";
                    ...
                    //  echo "2";
						...
						*/
						 ...
						 list($buy_return,$buy_error)=make_order_on_market($exchange1,$market1_force_type,$market1_act_type,$market1_price,$target_volume,$market1_price, $market1_pairs,'',$sell_return);
						if( strlen($buy_return)==19) {
						// if ($buy_return!="") {
							  list($sell_return,$sell_error)=make_order_on_market($exchange2,$market2_force_type,$market2_act_type,$market2_price,$target_volume,$market2_price, $market2_pairs,'',$buy_return);
                          ...
                          $json_return_params['tx_id2']=$sell_return;
                            ...
                            //  $json_return_params['status']=$arbi_type . " " . $target_gain_rate.  " -> : M1: ". $buy_error;
                              ...
                              $json_return_params['explanation']=  "1: " .$json_return_params['tx_id1'] . " 2: ".  $json_return_params['tx_id2'] "M1: ". $buy_error;
